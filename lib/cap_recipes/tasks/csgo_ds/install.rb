...
Capistrano::Configuration.instance(true).load do
  ...
  namespace :csgo_ds do
    ...
    set :csgo_ds_config_erb, File.join(File.dirname(__FILE__),'server.cfg.erb')
    set :csgo_ds_update_erb, File.join(File.dirname(__FILE__),'update_csgo.sh.erb')
    ...
    task :install, :roles => :csgo_ds do
      run "cd #{steamcmd_app_root} && ./steamcmd.sh +login anonymous +force_install_dir #{csgo_ds_source} +app_update 740 validate +quit"
    end

    task :update, :roles => :csgo_ds do
      run "cd #{steamcmd_app_root} && ./steamcmd.sh +runscript #{csgo_ds_source}/update_csgo.sh"
    end
    ...
    task :setup, :roles => :csgo_ds do
      ...
      utilities.sudo_upload_template csgo_ds_config_erb, "#{csgo_ds_source}/csgo/cfg/server.cfg", :owner => "#{steamcmd_user}:#{steamcmd_user}", :mode => "755"
      utilities.sudo_upload_template csgo_ds_update_erb, "#{csgo_ds_source}/update_csgo.sh", :owner => "#{steamcmd_user}:#{steamcmd_user}", :mode => "755"
